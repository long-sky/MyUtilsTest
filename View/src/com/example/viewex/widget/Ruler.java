...
import com.example.viewex.util.FloatUtil;
...
public class Ruler extends View{
	...
	private float getCurrentFreq(){
		int unitCount = Math.abs((int)FloatUtil.divide(mScrollDistance, 15));
		float freq = 0;
		switch (mRUlerType) {
		case RULER_TYPE_FM:
			freq =  FloatUtil.add(2.7f, FloatUtil.add(START_FREQ_FM, FloatUtil.mulitiply(unitCount, FloatUtil.divide(SCALE_UNIT_FM, 10f)))) ;
			break;
		case RULER_TYPE_AM:
			freq =  FloatUtil.add(27, FloatUtil.add(START_FREQ_AM, FloatUtil.mulitiply(unitCount, FloatUtil.divide(SCALE_UNIT_AM, 10f)))) ;
			break;
		default:
			break;
		}
